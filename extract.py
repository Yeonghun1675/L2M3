...
def run(publisher="ACS", config_num="1"):
    ...
    files = sorted(list(Path(f"../L2M3_data/0_csd_related_papers/{publisher}_{extension}/").glob(f"*.{extension.lower()}")))
    outfolder = Path(f"/home/users/ws1715/Projects/L2M3/{publisher}_results/")

    for filepath in tqdm(files[6800:]):
